...
class Qwen3ClientHead(PreTrainedModel):
    ...
    def _load_weight_from_pretrained_model_physically(self, pretrained_model: Qwen3ForCausalLM, from_l, to_l):
        ...
        hidden_layers = pretrained_model.model.layers
        hidden_layers: List[Qwen3DecoderLayer]
        self.layers = nn.ModuleList([Qwen3DecoderLayer(self.config, layer_idx) for layer_idx in range(to_l - from_l)])
        for i in range(from_l, to_l):
...
class Qwen3ClientTail(PreTrainedModel):
    def __init__(self, config: Qwen3Config, split_config: SplitModelConfig):
        super().__init__(config)
        self.split_config = split_config
        self.layers = None
    ...
    def _load_weight_from_pretrained_model_physically(self, pretrained_model: Qwen3ForCausalLM, from_l, to_l):
        ...
        for i in range(from_l, to_l):
            self.layers[i - from_l].load_state_dict(hidden_layers[i].state_dict())
        self.norm = Qwen3RMSNorm(pretrained_model.config.hidden_size, eps=pretrained_model.config.rms_norm_eps)
...
def _update_causal_mask(
    partitioned_model: Qwen3PreTrainedModel,
...
# 首先从与训练模型中加载模型
def load_qwen3_client(pretrained_model: Qwen3ForCausalLM, split_config: SplitModelConfig) -> Tuple[Qwen3ClientHead, Qwen3ClientTail]:
    ...
    head_model = Qwen3ClientHead(config, split_config)
    head_model.load_from_pretrained_model(pretrained_model, logical=split_config.logicl_load)
    tail_model = Qwen3ClientTail(config, split_config)
